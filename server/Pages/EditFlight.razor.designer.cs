...
{
    public partial class EditFlightComponent : ComponentBase
    {
        ...
        {
            var airlineReservationSystemsDbGetFlightByIdResult = await AirlineReservationSystemsDb.GetFlightById(Id);
            flight = airlineReservationSystemsDbGetFlightByIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(AirlineReservationSystems.Models.AirlineReservationSystemsDb.Flight args)
        {
            try
            {
                var airlineReservationSystemsDbUpdateFlightResult = await AirlineReservationSystemsDb.UpdateFlight(Id, flight);
                DialogService.Close(flight);
            }
            catch (System.Exception airlineReservationSystemsDbUpdateFlightException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Flight" });
            }
        }
